...
def main():
    ...
    from model.background_flow45_edit import configure_background_flow as configure_visual_flow
    ...
    while time.monotonic()-started < args.max_seconds and not (args.queue/'STOP').exists():
        ...
        try:
            ...
            source_rgb, _ = read_video_window(refs[1], 1, 1344, 768, 'head')
            source_image = destination/'source_appearance.png'
            Image.fromarray(source_rgb[0,:round(source_rgb.shape[1]*.45)]).save(source_image)
            ...
            result.update(timing=timing, conditioning=text_time, flow_edit=denoise.audit, fixed_head=False, appearance_crop_fraction=.45,
